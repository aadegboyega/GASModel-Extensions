...
StaticStarting_Uni <- function(vY, Dist, iK) {
  ...
  if (Dist == "gumbel") {
    ...
    dBeta = 0.7797* (dSigma2^0.5)
    dAlpha = dMean - (0.45*dSigma2^0.5)
    vTheta = c(dAlpha, dBeta)
  ...
  if (Dist == "weibull") {
	...
	med = median(vY)
	quarts <- quantile(vY, c(0.25, 0.75))

	dAlpha = log(log(0.25)/log(0.75))/log(quarts[2]/quarts[1])
	dBeta = med/log(2)^(1/dAlpha)

	
	
    #dBeta = dMean/dSigma2
    #dAlpha = dMean^2/dSigma2
	#dAlpha = 0.58 * (dMean^0.5)
	
	
	#dAlpha = ((dSigma2^0.5)/dMean)^-1.086
	#dBeta = dMean/gamma(1.0 + (1.0/dAlpha))
